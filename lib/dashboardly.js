...
class DashboardlyDataLoader {
    ...
    modifyActiveContract(user, modifiedContractData, contractId){
        //Step 1: update the current version is_latest to 0
        // will probably need a new field is_modified in contracts table
        ...
        const  qry =
            knex('contracts')
                ...
                .toString();
        //update the contract to cancelled with above query
        //console.log("The qry is",qry);
        return this.query(qry)
            .then(result => {
                //Step 2: get values from the old contract that I will need
                const qry2 =
                    knex
                        //Get Everything that wont be changed, Need to update Apiary
                        .select('contract_id','parent_id', 'payee_id', 'payee_id')
                        .from('contracts')
                        .where({
                            contract_id: contractId
                        })
                        .toString();
                console.log("The qry is",qry2);
                return this.query(qry2);
            })
            .then(oldContract => {
                //Step 3: Create the new contract version
                const qry3 =
                    knex
                        .insert({
            ...
            .then(insertResult => {
                //Step 4: Save the insertId and check notifications blacklist needed for next step
                ...
                const qry4 =
                    knex
                        ...
                        .toString();
                console.log("qr4 is", qry4); //see this on console. Very Good query
                return this.query(qry4);
            })
            .then(result => {
                //Step 5: Send notification to the payer
                console.log("Result from contract select= ", result)
                if (result.length){
                    const  qry5 =
                        knex
                            ...
                            .into('notifications')
                            .toString();
                    console.log("qr5 is", qry5);
                    return this.query(qry5);
                }
                return 0;
            })
            .then(()=>{
                //Step 6: Send the new Contract as response using the saved insertId in newContract Id
                return this.query(
                    knex

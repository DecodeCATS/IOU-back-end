...
class DashboardlyDataLoader {
    ...
    createNewContract(userId, contractData) {
        let insertChecker;
        ...
        return this.query(
            knex
                .insert({
                    ...
                    parent_id: contractData.parentId,
                    ...
                    accepted_date: contractData.acceptedDate,
                    contract_status: contractData.status
                })
                .into('contracts')
                .toString()
        )
        //Putting this at end, after taking care of notifications
        //and also avoiding nested promises
            // .then(contract => {
            //
            //     return this.query(
            //         knex
            //             .select('*')
            //             .from('contracts')
            //             .where('contract_id', contract.insertId)
            //             .toString()
            //     )
            ...
            // });
            .then(insertResult => {
                insertChecker = insertResult;
                /*Steps
                If insert is a success, send notifications to people
                on contacts lists and who havent blacklisted the user
                */
                if(insertChecker.insertId){
                  //insert stuff to notifications table after checking notification blacklist
                    return insertChecker; // or just the result of new insert query
                }
                else{
                    //you are working on develop, switch branch in morning
                    throw new Error ('Failed to create New Contract');
                }

            })
            .then(contract => {
                //return the stuff from contracts
                if(insertChecker.insertId){
                    return this.query(
                        knex
                            .select('*')

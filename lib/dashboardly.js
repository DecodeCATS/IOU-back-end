...
class DashboardlyDataLoader {
    ...
    // =======================================================================
    // *** Notification blacklist methods ***
    // =======================================================================
        ...
        return this.query(
            knex
                .select('users.user_id', 'users.username', 'users.first_name', 'users.last_name',
                    'users.user_type', 'users.created_at', 'users.updated_at', 'connections.is_blacklisted')
                .from('users')
                ...
                .where({
                    'connections.user1_id': user.users_user_id,
                    'connections.is_blacklisted': 1
                })
                .orderBy('connections.updated_at')
                .toString()
        )
                    ...
                    return this.query(
                        knex
                            .select('contract_id', knex.raw('COALESCE(parent_id, contract_id) as parent_id'), 'title',
                                ...
                                'payee_id', 'payer_id', 'contracts.is_latest', 'created_at', 'updated_at')
                            .from('contracts')
                            .where('contracts.contract_id', contractId)
                            .toString()
                    )
        ...
        const q =
            knex
                .select('contract_id', 'title', 'description', 'total_amount', 'remaining_amount', 'parent_id',
                    ...
                    'payer_id', 'payee_id', 'is_latest', 'created_at', 'updated_at')
                .where(function () {
    ...
    parseContractReceiverID(user, contractData){
        let receiverID;
        if (user.users_user_id === contractData.payeeId) {
            receiverID = contractData.payerId;
        }
        else if(user.users_user_id === contractData.payerId){
            receiverID = contractData.payeeId;
        }
        return receiverID;
    }

    createNewContractWithOrganisation(user, contractData){
        const qr =
            knex
                .insert({
    ...
    //use when the other connection is a person
    createNewContract(user, contractData) {
        ...
        const qr = knex
            .insert({
                ...
                payment_frequency: contractData.paymentFrequency,
                due_date: contractData.dueDate,
                contract_status: 'active' // TODO: Change back after testing
            })
            .into('contracts')
            .toString();
        ...
        return Promise.all([this.query(qr), this.checkIfUserIsBlacklisted(user.users_user_id, contractData.payerId),
            ...
            .then(result => {
                ...
                if (user.users_user_id === contractData.payeeId) {
                    ...
                    return this.query(
                        knex
                            .select('connections.user1_id', 'connections.is_blacklisted','users.username', 'users.user_type')
                            .from('connections')
                            .join('users', function () {
                                this.on('connections.user1_id', '=', 'users.user_id')
                            })
                            // .leftJoin('notifications_blacklist', function () {
                            //     this.on('connections.user2_id', '=', 'notifications_blacklist.blacklisted_id')
                            // })
                            .where({
                                'connections.user2_id': user.users_user_id,
                                'users.user_type': 'Person',
                                'connections.is_blacklisted': 0 //this is important see the raw version of this query using console.log
                            })
                            .toString()
                    )
                        .then(friends => {
        ...
        return this.query(
            knex
                .select('users.user_id', 'users.username', 'users.first_name',
                    ...
                    'users.updated_at', 'connections.is_blacklisted')
                .from('users')
                .join('connections', function () {
                    this.on('users.user_id', '=', 'connections.user2_id')
                })
    ...
    getAllOrganisationFromUser(userId) {
        return this.query(
            knex
                .select('users.user_id', 'users.username', 'users.first_name',
                    'users.user_type', 'users.created_at',
                    'users.updated_at', 'connections.is_blacklisted')
                .from('users')
                .join('connections', function () {
                    this.on('users.user_id', '=', 'connections.user2_id')
        ...
        return this.query(
            knex
                .select('users.user_id', 'users.username', 'users.first_name',
                    ...
                    'users.updated_at')
                .from('users')
                .where({'users.user_type':'Organisation'})
                .toString()
        )
            .then(connections => connections)
            .catch(err => err);
    ...
    // Returns 0 if no existing connection else returns 1
    checkIfConnectionExists(userId, friendId){
        const qry =
            knex
                .count('connection_id as isConnected')
                .from('connections')
                .where({
                    user1_id: userId,
                    user2_id: friendId
    ...
    // Use this with promise all to insert two values
    acceptConnection(user1Id, user2Id){
        const qry =
            knex
                .insert({
                    user1_id: user1Id,
    ...
    sendNotificationForAcceptedConnection(user, connectionData, result){
        const qry =
            knex
                .insert({
                    sender_id: user.users_user_id,
                    receiver_id: connectionData.userId,
                    object_id: result[0].insertId,
                    object_type: 'connections',
                    message: user.users_username + 'accepted your connection request'
                })
                .into('notifications')
                .toString();
        console.log(qry);
    ...
    acceptContractForUser(user, contractData) {
        // Step 1: Change the is_latest of the current active contract to 0 (Now the old version)
        const qry =
            knex('contracts')
                .where(function () {
                    this.where('contract_id', contractData.contractId)
                        .orWhere('parent_id', contractData.contractId)
                })
        ...
        return this.query(qry)
            ...
            .then(() => {
                const qry3 =
                    knex
                        .select('contract_id', 'parent_id', 'title', 'description', 'total_amount',
                            'number_of_Payments', 'payment_frequency', 'due_date', 'accepted_date',
                            'contract_status', 'is_latest', 'payer_id', 'payee_id', 'created_at',
                            'updated_at')
                        .from('contracts')
                        .where({
                ...
                return this.query(qry3)
                    ...
                    .then(contract => {
                        let acceptedContract = contract;
                        let sender;

                        if (user.users_user_id === contract[0].payee_id) {
                            sender = contract[0].payer_id;
                        }
                        else {
                            sender = contract[0].payee_id
                        }
                        ...
                        return this.query(
                            knex
                                .insert({
                                    sender_id: user.users_user_id,
                                    receiver_id: sender,
                                    object_id: contract[0].contract_id,
                                    object_type: 'contracts',
                                    notification_type: 'alert',
                            ...
                            .then(result => {
                                //return contract[0]; //TODO: might need fixing
    ...
    getAllPaymentsFromUser(user) {
      //Returning all payments for a single user now
        return this.query(
            knex
                .select('payments.payment_id', 'payments.contract_id',
                    ...
                    'payments.updated_at', 'contracts.parent_id')
                .from('payments')
    ...
    checkIfContractIsActive(userId, contractId){
        const qry =
            knex
                .select('*')
                .from('contracts')
        ...
        return this.query(qry)
            .then(result => {
                ...
                if (result.length === 0) {
                    throw new Error('You tried to make a payment for invalid contract');
                }
    ...
    addPaymentForContract(user, paymentData, receiverId) {
                ...
                const qry =
                    knex
                        .insert({
                            ...
                            object_id: insertResult.insertId,
                            object_type: 'payments',
                            notification_type: 'payment',
                            message: user.users_username + "has sent you a payment for a contract"
